...
public class CalculadoraHardware {
    ...
    private static void ejecutarEvaluacion(Scanner scanner) {
        try {
            ...
            String resultado = evaluarCuelloBotella(
                    nombreCPU, nucleosCPU, frecuenciaCPU, tdpCPU,
                    nombreGPU, memoriaGPU, velocidadGPU, tdpGPU
            );
            ...
            System.out.println("\n[ERROR] Se esperaba un numero entero valido.");
    ...
    // --------------------------------------------------------
    private static String evaluarCuelloBotella(
            String nombreCPU, int nucleos, int frecuencia, int tdpCPU,
            String nombreGPU, int memoria, int velocidad, int tdpGPU)
    ...
    private static int leerEnteroPositivo(Scanner scanner, String prompt)
        ...
        int valor = Integer.parseInt(entrada); // puede lanzar NumberFormatException
        if (valor <= 0) {
            throw new HardwareException(
